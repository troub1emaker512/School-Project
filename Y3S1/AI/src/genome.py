...
class Genome():
    ...
    def point_mutate(genome, rate, amount):
        """
        Point mutation:
        - With probability `rate` per gene value, add a random delta in [-amount, +amount]
        - Clamp values into [0.0, 0.9999]
        - Deep-copy genes so the original genome is not modified
        """
        amt = float(amount)
        if amt < 0:
            amt = -amt

        # Copy inner lists too (important: avoid mutating the parent genome)
        new_genome = [gene.copy() for gene in genome]

        for gene in new_genome:
            for i, val in enumerate(gene):
                if random.random() < rate:
                    delta = random.uniform(-amt, amt)
                    val = val + delta
    ...
    @staticmethod
    def to_csv(dna, csv_file, precision=6):
        """
        Save DNA to a readable CSV format.

        - One gene per line
        - Comma-separated values (no trailing comma)
        - Rounded to `precision` decimal places
        - Backwards compatible with from_csv() (it ignores empty values anyway)
        """
        lines = []
        for gene in dna:
            line = ",".join(f"{float(val):.{precision}f}" for val in gene)
            lines.append(line)
...
class URDFLink:
    ...
    def to_joint_element(self, adom):
        ...
        # PyBullet expects lower < upper; reversed limits can lock joints and prevent movement.
